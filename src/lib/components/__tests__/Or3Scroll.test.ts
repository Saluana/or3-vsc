...
describe('Or3Scroll', () => {
    ...
    it('compensates prepend with measured heights when loadingHistory', async () => {
        ...
        const prepend = Array.from({ length: 5 }, (_, i) => ({
            id: 1000 + i,
            text: `P ${i}`,
        }));
    ...
    it('updates rendered range after scroll', async () => {
        ...
        expect(Number(firstRendered.attributes('data-index'))).toBeGreaterThan(
            0
        );
    });

    it('keeps tail items rendered when approaching bottom with small overscan', async () => {
        const tailItems = Array.from({ length: 20 }, (_, i) => ({
            id: i,
            text: `Tail ${i}`,
        }));
        ...
        const renderedIndexes = wrapper
            .findAll('.or3-scroll-item')
            .map((el) => Number(el.attributes('data-index')));
        expect(renderedIndexes).toContain(tailItems.length - 1);
        expect(Math.min(...renderedIndexes)).toBeLessThanOrEqual(
            tailItems.length - 3
        );
    });

    it('applies scroll compensation immediately during user interaction', async () => {
        const wrapper = mount(Or3Scroll, {
            props: {
                items,
                itemKey: (item: any) => item.id,
        ...
        // Trigger resize for item 0 (simulating background update)
        // This item is now above the viewport (since we scrolled to 500 and item 0 is at 0-50)
        resizeCallbackAtIndex0({
            borderBoxSize: [{ blockSize: 100 }],
        } as unknown as ResizeObserverEntry);

        // Wait for microtasks (flushUpdates)
        await nextTick();

        // Should have applied compensation immediately
        // Original scrollTop 500 + delta (100 - 50 = 50) = 550
        expect(container.element.scrollTop).toBe(550);
