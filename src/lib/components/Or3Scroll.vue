...
export interface Props<T> {
    ...
    tailCount?: number;
    /**
     * Extra padding at the bottom of the scrollable area in pixels.
     * Useful for clearing floating elements like input bars.
     */
...
const scrollToIndex = (
    ...
    const measuredHeight =
        itemRefs.get(index)?.getBoundingClientRect().height ??
        props.estimateHeight ??
        50;
    const behavior = opts.smooth ? 'smooth' : 'auto';

    // Adjust for paddingTop
    let top = offset + props.paddingTop;

    if (opts.align === 'center') {
        top = top - (viewportHeight.value / 2 - measuredHeight / 2);
    } else if (opts.align === 'end') {
        top = top - (viewportHeight.value - measuredHeight);
    }
...
const onViewportResize = (newHeight: number) => {
    ...
    if (isAtBottom.value && !isUserScrolling.value) {
        ...
        container.value.scrollTop = Math.max(
            0,
            totalHeight + props.paddingTop + props.paddingBottom - newHeight
        );
        updateRange();
    } else {
...
watch(
    ...
    async (newItems, oldItems) => {
        ...
        if (newCount > oldCount) {
            ...
            const indexInNew = newItems.findIndex(
                (item) => getItemKey(item) === firstKeyOld
            );

            if (indexInNew > 0) {
                // Confirm that the tail matches to reduce false positives on reorder
                const tailMatches = oldItems.every(
                    (item, idx) =>
                        getItemKey(item) ===
...
<template>
    ...
    >
        <div
            ...
            :style="{ height: totalHeight + paddingTop + paddingBottom + 'px' }"
        >
            <div
